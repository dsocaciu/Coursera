...
if __name__ == '__main__':
	...
	df_holdings = copy.deepcopy(df_close)
	df_holdings = df_holdings * 0

	df_fund = copy.deepcopy(df_close)
	df_fund = df_fund * 0
...
#f_symprice_today = df_close[s_sym].ix[ldt_timestamps[i]]
			...
			df_fund[s_sym].ix[ldt_timestamps[d]] = float(df_holdings[s_sym].ix[ldt_timestamps[d]] *  df_close[s_sym].ix[ldt_timestamps[d]])
			#print ("Cash: " + str(float_port))
			#print("Fund " + s_sym + " " + str(df_fund[s_sym].ix[ldt_timestamps[d]]))
		...
		for s_sym in ls_symbols:
			float_fund += np.nan_to_num(df_fund[s_sym].ix[ldt_timestamps[d]])
